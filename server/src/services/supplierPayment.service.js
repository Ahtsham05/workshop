...
const sumFilteredAmount = async (filter) => {
  const docs = await SupplierPayment.find(filter).select('amount').lean();
...
const queryPayments = async (filter, options) => {
  ...
  const [result, totalAmountSum] = await Promise.all([SupplierPayment.paginate(filter, opts), sumFilteredAmount(filter)]);
