...
const SYSTEM_PROMPT = `You are an OCR assistant for a business management app. Extract customer records from images of handwritten or printed customer lists, notebooks, ledgers, contact sheets, ID cards, or screenshots.

Return ONLY valid JSON with this exact shape (no markdown, no explanation):
{"customers":[{"name":"","nameUrdu":"","email":"","phone":"","whatsapp":"","address":"","balance":0}]}

Rules:
- Extract every distinct customer row you can read
...
function parseJsonFromContent(content) {
  ...
  try {
    ...
    throw new ApiError(httpStatus.BAD_GATEWAY, 'Could not parse customer data from image');
  }
}
...
async function fillEnglishNamesFromUrdu(apiKey, customers) {
  const pending = customers
    .map((c, index) => ({ index, nameUrdu: c.nameUrdu }))
    .filter((item) => item.nameUrdu && !customers[item.index].name);
  ...
  const prompt = `Romanize these Pakistani/Urdu person names into Latin English for a customer database.
...
${lines}`;
  ...
  try {
    ...
    pending.forEach((item, i) => {
      ...
      if (english && /[A-Za-z]/.test(english)) {
        customers[item.index].name = english;
      }
    });
  } catch {
  ...
  await Promise.all(
    pending.map(async (item) => {
      if (customers[item.index].name) return;
    ...
    contents: [
      {
        parts: [
          {
            text: `${SYSTEM_PROMPT}\n\nExtract all customers from this image. Return JSON only.`,
          },
          {
            inline_data: {
              mime_type: safeMime,
...
/**
 * Extract customer rows from an image buffer using Google Gemini vision (API v1).
 * @param {Buffer} imageBuffer
 * @param {string} mimeType
 * @returns {Promise<{ customers: Array, modelUsed: string }>}
 */
async function extractCustomersFromImage(imageBuffer, mimeType = 'image/jpeg') {
  ...
  const list = Array.isArray(parsed?.customers) ? parsed.customers : Array.isArray(parsed) ? parsed : [];

  let customers = list.map(normalizeCustomer).filter(Boolean);

  if (customers.length === 0) {
    throw new ApiError(
      httpStatus.UNPROCESSABLE_ENTITY,
      'No customers could be read from this image. Try a clearer photo with visible names.',
    );
  }

  customers = await fillEnglishNamesFromUrdu(apiKey, customers);

  return { customers, modelUsed };
}
