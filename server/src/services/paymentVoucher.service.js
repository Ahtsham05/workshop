...
const sumFilteredAmount = async (filter) => {
  const docs = await PaymentVoucher.find(filter).select('totalAmount').lean();
...
const queryVouchers = async (filter, options) => {
  ...
  const [result, totalAmountSum] = await Promise.all([PaymentVoucher.paginate(filter, opts), sumFilteredAmount(filter)]);
