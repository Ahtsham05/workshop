...
import { addSupplier } from '@/stores/supplier.slice'
...
/** Fast, no-photos supplier entry — sits beside the list for one-line-at-a-time
 *  data entry. The full "Add Supplier" header button still opens the complete
 *  dialog (Urdu name, photos, ID uploads) for when those are needed. */
export function QuickAddSupplierCard({ setFetch, onCreated }: Props) {
  ...
  // Auto-copy phone to WhatsApp, same UX as the full Add Supplier dialog.
  useEffect(() => {
  ...
  const onSubmit = async (values: QuickAddForm) => {
    ...
    try {
      const created = await dispatch(addSupplier(values)).unwrap()
      toast.success(t('supplier_created_success'))
      setFetch?.((prev) => !prev)
      onCreated?.(created)
      form.reset(emptyValues)
    } catch {
      // addSupplier already surfaces its own error toast
    } finally {
    ...
    <div className='rounded-xl border bg-card p-5 shadow-sm'>
      <div className='mb-4 flex items-center gap-3'>
        ...
        <div className='min-w-0'>
          <p className='truncate text-sm font-semibold'>{t('quick_add_supplier')}</p>
          <p className='truncate text-xs text-muted-foreground'>{t('quick_add_supplier_desc')}</p>
        </div>
      </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='gap-1.5'>
                <FormLabel className='text-xs'>{t('supplier_name')}</FormLabel>
                <FormControl>
                  <Input placeholder={t('enter_supplier_name')} autoComplete='off' showVoiceInput={false} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
