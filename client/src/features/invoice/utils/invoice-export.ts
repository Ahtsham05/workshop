...
const EXPORT_COLUMNS: { header: string; value: (row: InvoiceExportRow) => string | number }[] = [
  ...
  { header: 'Customer', value: (row) => row.customerDisplayName || '' },
  { header: 'Items', value: (row) => row.itemsCount ?? 0 },
  { header: 'Invoice Date', value: (row) => (row.invoiceDate ? format(new Date(row.invoiceDate), 'yyyy-MM-dd') : '') },
  ...
  { header: 'Invoice Total', value: (row) => Number(row.total || 0).toFixed(2) },
...
const timestampedName = (extension: string) => `invoices-${format(new Date(), 'yyyy-MM-dd-HHmm')}.${extension}`

export function exportInvoicesToCsv(rows: InvoiceExportRow[]) {
  const csv = [
    EXPORT_COLUMNS.map((column) => csvCell(column.header)).join(','),
    ...rows.map((row) => EXPORT_COLUMNS.map((column) => csvCell(column.value(row))).join(',')),
  ].join('\n')

  // A UTF-8 BOM so Excel reads the file as UTF-8 — Urdu customer names otherwise arrive as mojibake.
 ...
 * path in this app uses — no PDF library is bundled, and the print dialog gives the user
 * page size and orientation for free. Mirrors purchase-export.ts's exportPurchasesToPdf.
 */
...
<html><head><meta charset="utf-8"><title>${escapeHtml(meta.title || 'Invoice Report')}</title>
<style>
