...
const INVOICE_TYPE_OPTIONS = [
  ...
  { value: 'pending-converted', label: 'Converted pending' },
]
...
/**
 * Checkbox multi-select in a popover. Mirrors purchase-filters-toolbar.tsx's identical
 * component — every advanced filter on this toolbar needs "any of these".
 */
...
export interface InvoiceFiltersToolbarProps {
  filters: InvoiceFilters
  draft: InvoiceFilters
  activeFilterCount: number
  onPatchImmediate: (patch: Partial<InvoiceFilters>) => void
  onPatchDraft: (patch: Partial<InvoiceFilters>) => void
  onApplyDraft: () => void
  onReset: () => void
...
export function InvoiceFiltersToolbar({
  ...
  // The picker only needs the customer list once the panel is actually opened — filters are
  // a secondary action, so fetching every customer on every list render would be waste.
  const { data: customersData } = useGetAllCustomersQuery(undefined, { skip: !advancedOpen })
  ...
  const activePills = useMemo(() => {
    ...
    if (filters.customer.length)
      pills.push({ key: 'customer', label: `Customer: ${labelFor(customerOptions, filters.customer)}`, clear: () => onPatchImmediate({ customer: [] }) })
    ...
    return pills
  }, [filters, userOptions, customerOptions, branches, onPatchImmediate])
  ...
  return (
    <Card className='overflow-hidden'>
      <CardContent className='space-y-4 p-4'>
        {/* Search + type + sort + actions */}
        <div className='flex flex-col gap-3 lg:flex-row lg:items-center'>
          <div className='flex flex-1 items-center gap-2'>
            <Select
              value={filters.searchBy}
              onValueChange={(value) => onPatchImmediate({ searchBy: value as InvoiceSearchField })}
            >
            ...
            <div className='relative flex-1'>
              ...
              <Input
                ...
                placeholder={t('Search invoices, customers, products, bills...')}
                className='pl-9 pr-9'
              />
              {filters.search && (
                <button
          ...
          <div className='flex flex-wrap items-center gap-2'>
            <Select value={filters.type || 'all'} onValueChange={(value) => onPatchImmediate({ type: value === 'all' ? '' : value })}>
              <SelectTrigger className='w-[160px]' aria-label={t('Invoice Type')}>
          ...
          >
            ...
            <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4'>
              <MultiSelectFilter
                ...
                onChange={(values) => onPatchDraft({ customer: values })}
              />
              <MultiSelectFilter
