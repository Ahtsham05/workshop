...
export type InvoiceSearchField = 'all' | 'invoice' | 'customer' | 'product' | 'billNumber' | 'notes'

export const SEARCH_FIELD_OPTIONS: { value: InvoiceSearchField; label: string }[] = [
...
export interface InvoiceFilters {
  search: string
  searchBy: InvoiceSearchField
  /** Invoice Type — a single value from the existing dropdown (all/cash/credit/pending/quotation/pending-converted). */
  type: string
  customer: string[]
 ...
 * whatever else is set rather than wiping the toolbar. Mirrors use-purchase-filters.ts's
 * QUICK_FILTERS — Cash/Credit chips are deliberately left out here since the existing
 * "Invoice Type" dropdown already covers that need.
 */
export const QUICK_FILTERS: { id: string; label: string; patch: () => Partial<InvoiceFilters> }[] = [
...
export function isQuickFilterActive(id: string, filters: InvoiceFilters): boolean {
  const preset = QUICK_FILTERS.find((quick) => quick.id === id)
  if (!preset) return false
  return Object.entries(preset.patch()).every(([key, value]) => {
    const current = filters[key as keyof InvoiceFilters]
...
export function buildInvoiceQueryParams(filters: InvoiceFilters): Record<string, string | number> {
  ...
  if (filters.customer.length) params.customerId = list(filters.customer)
...
/** How many advanced filters (ignoring search, sort and the Invoice Type dropdown) are
 *  narrowing the list right now — drives the "Filters" button's badge count. */
export function countActiveFilters(filters: InvoiceFilters): number {
  let count = 0
  count += filters.customer.length ? 1 : 0
...
const SAVED_VIEWS_KEY = 'invoiceListSavedViews'
...
/**
 * Filter state for the invoice list. Mirrors use-purchase-filters.ts's usePurchaseFilters.
 *
 ...
 * the search box, Invoice Type dropdown, quick chips and sort controls write through to both
 * immediately — a half-typed amount range should never refetch, but switching type obviously
 * should.
 */
export function useInvoiceFilters() {
  const [filters, setFilters] = useState<InvoiceFilters>(EMPTY_FILTERS)
  const [draft, setDraft] = useState<InvoiceFilters>(EMPTY_FILTERS)
  ...
  const patchImmediate = useCallback((patch: Partial<InvoiceFilters>) => {
    ...
    (id: string) => {
      ...
      if (isQuickFilterActive(id, filters)) {
        ...
        const cleared = Object.fromEntries(
          Object.keys(patch).map((key) => [key, EMPTY_FILTERS[key as keyof InvoiceFilters]])
  ...
  return {
    ...
    draft,
    appliedFilters,
    queryParams: useMemo(() => buildInvoiceQueryParams(appliedFilters), [appliedFilters]),
    activeFilterCount: useMemo(() => countActiveFilters(filters), [filters]),
    setDraft,
    patchDraft: useCallback((patch: Partial<InvoiceFilters>) => setDraft((previous) => ({ ...previous, ...patch })), []),
    patchImmediate,
    applyDraft,
    resetAll,
    toggleQuickFilter,
