...
export interface PrintableVoucherLine {
  payeeType: 'expense' | 'supplier' | 'other'
  category?: string
  supplierName?: string
  payeeName: string
  amount: number
...
export interface PrintablePaymentVoucher {
  voucherNumber: string
  date: string
  bankAccountName?: string
  lines: PrintableVoucherLine[]
  totalAmount: number
  reference?: string
  notes?: string
}
...
const generateVoucherHTML = (voucher: PrintablePaymentVoucher, company: PrintCompany, paperSize: PaperSize, orientation: PrintOrientation) => {
...
<head>
  <meta charset="UTF-8">
  <title>Payment Voucher ${escapeHtml(voucher.voucherNumber)}</title>
  <style>
    @media print {
      @page {
    ...
    .print-btn-primary { background: #b91c1c; color: white; }
  ...
  <table class="lines-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>${escapeHtml(PAYEE_TYPE_LABEL[line.payeeType] || line.payeeType)}</td>
...
export const printPaymentVoucher = (
  voucher: PrintablePaymentVoucher,
