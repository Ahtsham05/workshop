...
interface CustomerAiScanDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onImport: (customers: Record<string, string | number>[]) => Promise<void>
}
...
function createEmptyRow(): ScannedCustomer {
  return {
...
function mapApiCustomer(c: Record<string, unknown>, index: number): ScannedCustomer {
...
function rowHasName(row: ScannedCustomer) {
  return Boolean(row.name.trim() || row.nameUrdu.trim())
}

const ROW_NAV_FIELDS: (keyof ScannedCustomer)[] = [
  'name',
  'nameUrdu',
  'balance',
  'email',
...
async function enrichRowsWithEnglish(rows: ScannedCustomer[]): Promise<ScannedCustomer[]> {
...
export function CustomerAiScanDialog({
  open,
  onOpenChange,
  onImport,
}: CustomerAiScanDialogProps) {
  const { t } = useLanguage()
  const fileInputRef = useRef<HTMLInputElement>(null)
  const translateTimersRef = useRef<Record<string, ReturnType<typeof setTimeout>>>({})
  const pendingFocusRef = useRef<{ rowId: string; field: keyof ScannedCustomer } | null>(
    null,
  )
  ...
  const [rows, setRows] = useState<ScannedCustomer[]>([])
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/customers/scan-image`,
        {
          method: 'POST',
          body: formData,
          headers: {
      ...
      if (customers.length === 0) {
        toast.error(t('no_customers_found_in_image'))
        return
      }

      const mapped = customers.map(mapApiCustomer)
      setRows(await enrichRowsWithEnglish(mapped))
      setStep('review')
      toast.success(
        `${t('ai_scan_success')}: ${customers.length} ${t('customers_found')}`,
      )
    } catch (error) {
  ...
  const updateRow = (id: string, field: keyof ScannedCustomer, value: string) => {
    setRows((prev) =>
  ...
  const focusRowField = useCallback((rowId: string, field: keyof ScannedCustomer) => {
  ...
  const handleImport = useCallback(async () => {
    const customers = buildImportPayload()
    if (!customers) return
    ...
    try {
      ...
      toast.success(
        `${t('import_successful')}: ${customers.length} ${t('customers_imported')}`,
      )
      resetDialog()
      onOpenChange(false)
    } catch (error) {
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden px-3 py-3 sm:px-4 sm:py-4">
          <DialogHeader className="shrink-0 space-y-1 text-left pb-2">
            ...
            <DialogDescription className="text-xs sm:text-sm">
              {t('ai_scan_customers_description')}
            </DialogDescription>
          </DialogHeader>
            ...
            {step === 'upload' && (
              <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 overflow-y-auto">
                <Alert>
                  ...
                  <AlertDescription>{t('ai_scan_tip')}</AlertDescription>
                </Alert>
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-medium">
                      {t('review_and_edit_customers')}: {rows.length}
                    </p>
