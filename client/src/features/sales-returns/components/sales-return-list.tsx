...
export default function SalesReturnList({ onCreateNew }: SalesReturnListProps) {
  ...
  const { data, isLoading } = useGetSalesReturnsQuery(queryParams)
  const [updateStatus] = useUpdateSalesReturnStatusMutation()
              ...
              {isLoading ? (
                ...
                returns.map((ret) => (
                  <TableRow key={ret._id || ret.id}>
                    ...
                    <TableCell>
                      {typeof ret.invoiceId === 'object'
                        ? (ret.invoiceId as any)?.invoiceNumber
                        : ret.invoiceId}
                    </TableCell>
