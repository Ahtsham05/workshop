 ...
 *
 * Same split as the sign-in/sign-up screens' `AuthLayout`, and pinned the same way
 * regardless of the account's own light/dark preference: the rail is always dark,
 * the content card is always light, with a black CTA (`CTA_BUTTON_CLASS`) — not a
 * light/dark pair that follows `<html>`. See `AuthLayout`'s own comment for why the
 * `dark`/`light` scoping classes and the `contents` trick on the rail wrapper work
 * regardless of what `<html>` currently has.
 */
export function OnboardingShell({ steps, current, children }: Props) {
  const percent = Math.round(((current + 1) / steps.length) * 100)

  return (
    <div className='light bg-background grid min-h-svh lg:grid-cols-[320px_1fr]'>
      {/* Desktop rail */}
      <div className='dark contents'>
        <aside className='relative hidden overflow-hidden bg-[linear-gradient(165deg,#eef2ff_0%,#f5f7ff_45%,#eff6ff_100%)] p-10 text-slate-400 dark:bg-[linear-gradient(160deg,#050508_0%,#0c0e18_55%,#161a2c_100%)] lg:flex lg:flex-col'>
          <div
                  ...
                  <div className='flex flex-col items-center'>
                    <span
                      className={cn(
                        ...
                        active &&
                          'border-transparent bg-gradient-to-br from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-600/30 dark:from-white dark:to-white dark:text-[#0d0f16] dark:shadow-none',
                        !done &&
                          !active &&
      ...
      {/* Content */}
      <main className='light text-foreground flex flex-col'>
        {/* Phone progress */}
        ...
        <div className='flex flex-1 items-center justify-center px-5 py-8 sm:px-8'>
          <div className='w-full max-w-xl lg:rounded-2xl lg:border lg:bg-card lg:p-8 lg:shadow-xl lg:shadow-slate-900/[0.06]'>
            {children}
          </div>
        </div>
      </main>
