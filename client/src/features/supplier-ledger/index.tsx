...
type SupplierLedgerFormValues = z.infer<typeof supplierLedgerSchema>;

export default function SupplierLedger() {
  const [fetching, setFetching] = useState(false);
  // Fallback to an empty array if suppliersOptions is null or undefined
  const suppliersOptions = useSelector((state: any) => state.supplier?.data) || [];

  const form = useForm<SupplierLedgerFormValues>({
    resolver: zodResolver(supplierLedgerSchema),
    defaultValues: {
      supplier: '',
      startDate: '',
      endDate: '',
  ...
  const onSubmit = async (values: SupplierLedgerFormValues) => {
    setFetching(true);
    try {
      navigate({ to: `/supplier-ledger-detail?supplier=${values.supplier}&startDate=${values.startDate}&endDate=${values.endDate}`, replace: true })
    } catch (error) {
      ...
      <Main>
        <div className="mb-2 flex flex-wrap items-center justify-between space-y-2">
          <div>
            ...
            <p className="text-muted-foreground">Manage your Supplier Ledger here.</p>
          </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    value={suppliersOptions?.find((o: any) => o?.value === field?.value)}
