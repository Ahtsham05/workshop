...
type CustomerLedgerFormValues = z.infer<typeof customerLedgerSchema>;
...
export default function CustomerLedger() {
  ...
  const form = useForm<CustomerLedgerFormValues>({
  ...
  const onSubmit = async (values: CustomerLedgerFormValues) => {
    setFetching(true);
    try {
      navigate({ to: `/customer-ledger-detail?customer=${values.customer}&startDate=${values.startDate}&endDate=${values.endDate}`, replace: true })
    } catch (error) {
      ...
      <Main>
        <div className="mb-2 flex flex-wrap items-center justify-between space-y-2">
          <div>
            ...
            <p className="text-muted-foreground">Manage your Customer Ledger here.</p>
          </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    value={customersOptions?.find((o: any) => o?.value === field?.value)}
