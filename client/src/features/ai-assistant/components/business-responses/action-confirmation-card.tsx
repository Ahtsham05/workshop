...
/**
 * Renders a create_invoice preview as a bound Cancel/Confirm card. Nothing is written to the
 * database until Confirm is clicked — see aiAssistant.service.js#confirmAction, which
 * re-validates permission and re-resolves the customer/product server-side before calling
 * invoiceService.createInvoice. `status` comes straight from the persisted message, so a
 * page reload shows the same executed/cancelled/failed state instead of resetting to pending.
 */
export function ActionConfirmationCard({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const message = (err as { data?: { message?: string } })?.data?.message || 'Failed to create the invoice.'
    ...
    return (
      <div className='mt-2 w-full max-w-sm rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4 shadow-sm'>
        ...
        <p className='mt-1 text-sm text-foreground'>
          {action.result?.invoiceNumber} — {formatMoney(preview.total)} for {preview.customerName}
        </p>
    ...
    <div data-testid='action-confirmation-card' className='mt-2 w-full max-w-sm rounded-2xl border bg-card p-4 shadow-sm'>
      ...
      <dl className='mt-2 space-y-1.5 text-sm'>
        <div className='flex justify-between gap-3'>
          ...
          <dd className='truncate font-medium'>{preview.customerName}</dd>
        </div>
        <div className='flex justify-between gap-3'>
          <dt className='text-muted-foreground'>Product</dt>
          <dd className='truncate font-medium'>{preview.productName}</dd>
        </div>
        <div className='flex justify-between gap-3'>
          <dt className='text-muted-foreground'>Quantity</dt>
          <dd className='font-medium'>{preview.quantity}</dd>
        </div>
        <div className='flex justify-between gap-3 border-t pt-1.5'>
          <dt className='text-muted-foreground'>Total</dt>
          <dd className='font-semibold tabular-nums'>{formatMoney(preview.total)}</dd>
        </div>
      </dl>
      ...
      <div className='mt-3 flex gap-2'>
        ...
        <Button size='sm' className='h-8 flex-1 text-xs' onClick={handleConfirm} disabled={busy}>
          {isConfirming ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : 'Create Invoice'}
        </Button>
      </div>
    </div>
  )
