...
import { DIRECTION_BADGE_CLASS, DIRECTION_LABELS, STATUS_BADGE_CLASS, STATUS_LABELS, toSupplierPaymentSheetData } from '../utils/payment-sheet-data'
...
const partyName = (payment: SupplierPaymentRecord) =>
  (typeof payment.supplier === 'object' ? payment.supplier.name : undefined) || payment.supplierName || 'Unknown supplier'

const ALL = '__all__'

export interface SupplierPaymentListInitialFilters {
  startDate?: string
  endDate?: string
  direction?: string
  status?: string
...
interface Props {
  initialFilters?: SupplierPaymentListInitialFilters
}

export function SupplierPaymentList({ initialFilters }: Props) {
  const formatMoney = useFormatMoney()
  const { hasAnyPermission } = usePermissions()
  const canManage = hasAnyPermission('deletePayments', 'deletePurchases', 'manageLedgers')
  ...
  const [paymentToVoid, setPaymentToVoid] = useState<SupplierPaymentRecord | null>(null)
  ...
  const { data, isLoading } = useGetSupplierPaymentsQuery(queryArgs)
  ...
  const { currentData: detail, isFetching: isFetchingDetail } = useGetSupplierPaymentQuery(viewingId as string, {
    skip: !viewingId || !viewOpen,
  })
  const viewingRecord: SupplierPaymentRecord | undefined = detail ?? payments.find((p) => p.id === viewingId)

  const [triggerExport, { isFetching: isExporting }] = useLazyGetSupplierPaymentsQuery()
  const [voidPayment, { isLoading: isVoiding }] = useVoidSupplierPaymentMutation()

  const openView = (payment: SupplierPaymentRecord) => {
  ...
  const handleExport = async (fileFormat: 'xlsx' | 'csv') => {
    ...
    if (selected.length > 0) {
      exportPaymentsToExcel(selected.map(toSupplierPaymentSheetData), { kind: 'supplier', partyColumnLabel: 'Supplier', format: fileFormat })
      return
    }
    try {
      const result = await triggerExport({ ...queryArgs, page: undefined, limit: 5000 }).unwrap()
      exportPaymentsToExcel(result.results.map(toSupplierPaymentSheetData), { kind: 'supplier', partyColumnLabel: 'Supplier', format: fileFormat })
    } catch {
      toast.error('Failed to export supplier payments')
    }
  }
  ...
  return (
    <div className='space-y-4'>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className='flex h-40 flex-col items-center justify-center gap-2'>
              ...
              <p className='text-muted-foreground'>No supplier payments found for these filters.</p>
            </div>
          ) : (
            <>
      ...
      <PaymentDetailSheet
        kind='supplier'
        payment={viewingRecord ? toSupplierPaymentSheetData(viewingRecord) : null}
        open={viewOpen}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This reverses every purchase bill this payment settled back to outstanding, and reverses its effect on the bank
              account balance and cash book. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Textarea
